...
const shouldTag = (name) => {
    ...
    // Avoid tagging any component that looks like a Three.js or R3F component
    const threeDKeywords = ['Sphere', 'Light', 'Material', 'Controls', 'Geometry', 'Mesh', 'Camera', 'Float', 'Group', 'Box', 'Canvas', 'AnimatedSphere'];
    if (threeDKeywords.some(keyword => name.endsWith(keyword))) {
        return false;
    }
    // Standard Three.js elements are lowercase but not standard HTML tags (mostly)
    const htmlTags = ["div", "span", "a", "button", "img", "p", "h1", "h2", "h3", "h4", "h5", "h6", "section", "main", "header", "footer", "nav", "aside", "ul", "ol", "li", "input", "textarea", "label", "form", "select", "option", "canvas", "svg", "path", "circle", "rect", "line", "polyline", "polygon", "ellipse", "text", "g", "defs", "clippath", "mask", "use", "image", "video", "audio", "iframe", "table", "thead", "tbody", "tr", "th", "td", "details", "summary", "blockquote", "pre", "code", "em", "strong", "i", "b", "u", "small", "sub", "sup", "br", "hr", "picture", "source", "dialog", "menu", "template", "slot"];
    const isThreeElement = /^[a-z]/.test(name) && !htmlTags.includes(name);
    return !isThreeElement && !threeFiberElems.includes(name) && !dreiElems.includes(name);
};
